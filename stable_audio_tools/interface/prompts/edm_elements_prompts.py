"""Legacy Audialab EDM Elements prompt vocabulary and builder helpers.

The random generator below preserves the original weighted prompt behavior.
The manual builder exposes the same vocabulary as composable primitives.
"""
import random

NONE = "None"
SOUND_TAG_CHOICES = [
    "Pluck", "Sine", "Bright", "Clean", "Bell", "Saw", "Synth", "Warm",
    "Lead", "Supersaw", "Square", "Buzzy", "Legato", "Bass", "Punchy", "Sub",
]
STRUCTURE_CHOICES = ["arp", "melody", "chord progression"]
MUSICAL_TAG_CHOICES = [
    "slow speed", "medium speed", "fast speed", "alternating", "triplets",
    "rising", "falling", "repeating", "catchy", "bounce", "top", "simple",
    "complex", "off beat", "strummed", "dance", "pluck",
]
EFFECT_CHOICES = [
    "small reverb", "medium reverb", "high reverb",
    "with falling high-cut", "with rising low-pass",
    "with half-beat gate", "with quarter-beat gate",
]
...
def _dedupe(values):
    ...
    for value in values or []:
        ...
        if text and text.casefold() not in seen:
            seen.add(text.casefold()); out.append(text)
    return out

def build_prompt(sound_tags=None, structure=None, musical_tags=None, effects=None):
    tokens=[]
    tokens.extend(_dedupe(sound_tags))
    structure=str(structure or NONE).strip()
    if structure and structure.casefold()!=NONE.casefold():
        tokens.append(structure)
    tokens.extend(_dedupe(musical_tags))
    tokens.extend(_dedupe(effects))
    return ", ".join(_dedupe(tokens))

def prompt_generator_edm(*, return_plan=False):
    ...
    if return_plan:
        # Preserve the historical random prompt string, but also expose a
        # normalized builder plan so the UI can mirror the generated choices.
        lower_specific = " ".join(specific_descriptors).casefold()

        if "chord progression" in lower_specific or "progression" in lower_specific:
            structure = "chord progression"
        elif "arp" in lower_specific:
            structure = "arp"
        else:
            structure = "melody"

        musical_tags = []
        # Longer phrases first so `medium speed` does not degrade into `medium`.
        for tag in MUSICAL_TAG_CHOICES:
            if tag.casefold() in lower_specific:
                musical_tags.append(tag)

        # A few legacy phrases imply a structural/musical primitive without
        # containing the builder spelling verbatim.
        if "slow " in lower_specific and "slow speed" not in musical_tags:
            musical_tags.append("slow speed")
        if "dance" in lower_specific and "dance" not in musical_tags:
            musical_tags.append("dance")
        if "pluck" in lower_specific and "pluck" not in musical_tags:
            musical_tags.append("pluck")

        return {
            "prompt": descriptor,
            "sound_tags": list(initial_descriptors),
            "structure": structure,
            "musical_tags": _dedupe(musical_tags),
...
def prompt_generator(*, return_plan=False, **_):
    return prompt_generator_edm(return_plan=return_plan)
